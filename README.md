...
Лабораторные по численным методам: Streamlit-портал и две работы.

| Каталог   | Содержимое |
|-----------|------------|
| `portal/` | Портал [labs.inst.rest](https://labs.inst.rest): кнопки перехода в лаб. №1 и №2 |
...
# то же для lab1 и lab2 (отдельный venv в каждой папке)
./portal/run_local.sh
```
...
Кратко: клонировать репозиторий в `/opt/labs/numerical_labs`, создать `venv` в `portal`, `lab1`, `lab2`, подставить production-конфиги Streamlit из `portal/deploy/configs/`, включить юниты из `portal/deploy/systemd/`.
